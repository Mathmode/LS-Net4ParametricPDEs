...
from SRC.loss import training, u_lifted, relative_error_lower_and_upper
from SRC.callbacks import Validation
from SRC.config import Aparam, Bparam, MODELS_FOLDER, EPOCHS, XYPLOT, DTYPE
...
df_pred_constant.to_csv(f"{MODELS_FOLDER}/pred_constant.csv", sep=";", index=False)

# With pieceise-constant p
pred_materials = net_lifted([x, y, sigma_materials])
df_pred_materials = pd.DataFrame(tf.concat([x, y, pred_materials], axis=1), columns=["x", "y", "pred"])
df_pred_materials.to_csv(f"{MODELS_FOLDER}/pred_materials.csv", sep=";", index=False)
...
df_hist.to_csv(f"{MODELS_FOLDER}/histogram.csv", sep=";", index=False)
...
df_loss.to_csv(f"{MODELS_FOLDER}/loss.csv", sep=";", index=False)
...
df_val.to_csv(f"{MODELS_FOLDER}/validation.csv", sep=";", index=False)
